...
const skillData = [
  {
    type: "Frontend",
    fields: ["React.js", "HTML", "Redux", "CSS"],
  },
  {
    type: "Technical",
    fields: ["Data Structures and Algorithms", "Object Oriented Programming","DBMS"],
  },
  {
    type: "BACKEND",
  ...
  {
    type: "Others",
    fields: ["Photoshop", "Solid Works", "CAD"],
  },
];
      ...
      <div className="pglang-content">
        {pgData.map((data, index) => {
          ...
          for (let i = 1; i <= parseInt(data.rating); i++) {
            filledStars.push(<div className="starBox">{starFilled}</div>);
          }

          let unfilledStars = [];
          for (let i = 1; i <= 5 - parseInt(data.rating); i++) {
            unfilledStars.push(<div className="starBox">{starHollow}</div>);
          }

          return (
            <>
              <h2 className="card-company wc semi-bold m">{data.lang}</h2>
